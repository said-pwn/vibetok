      ...
      <header className="flex items-center justify-between p-4 bg-black/80 backdrop-blur-lg border-b border-pink-500/20 sticky top-0 z-50">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            title="Поиск"
          >
            <Search className="w-5 h-5 text-white group-hover:text-pink-400 transition-smooth" />
          </button>
          <button
            onClick={() => setShowNotifications(true)}
            className="relative p-2.5 hover:bg-pink-500/20 rounded-full transition-smooth group"
            title="Уведомления"
          >
            <Bell className="w-5 h-5 text-white group-hover:text-pink-400 transition-smooth" />
            {unreadCount > 0 && (
              <span className="absolute top-1 right-1 w-4 h-4 bg-pink-500 rounded-full flex items-center justify-center text-xs font-bold animate-pulse-custom">
                {unreadCount}
              </span>
            )}
          </button>
          <Link
            to="/messages"
            className="p-2.5 hover:bg-pink-500/20 rounded-full transition-smooth group relative"
            title="Сообщения"
          >
            <MessageCircle className="w-5 h-5 text-white group-hover:text-pink-400 transition-smooth" />
          </Link>
          <Link
            to="/upload"
            className="p-2.5 hover:bg-pink-500/20 rounded-full transition-smooth group"
            title="Загрузить"
          >
            <Plus className="w-5 h-5 text-white group-hover:text-pink-400 transition-smooth" />
          </Link>
          {userData && (
            <Link
              ...
              title="Профиль"
            >
              <User className="w-5 h-5 text-white group-hover:text-pink-400 transition-smooth" />
            </Link>
          )}
        </div>
      </header>
